...
prices_benchmark_daily_ret = prices_benchmark_daily_ret.to_frame()
...
prices = prices.iloc[126:]  # Remember that we are dropping na values: first 6 months
...
momentum['signal5'] = np.select(condition_5, ['TIP'])
...
trading_month_start = trading_month_start[6:]
...
final_df = pd.concat([daily_ret, result_df], axis=1)
...
final_df['signal'] = np.where(final_df['signal'] == 'TIP', final_df['TIP'], final_df['signal'])
final_df['signal'] += 1
